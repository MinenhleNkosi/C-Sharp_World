...
flowchart TD
    ...
    C --> C3[backslash-backslash - Literal backslash]
    C --> C4[backslash-quote - Double quote character]
    C1 --> C1a[Output: Line 1 then Line 2 on next line]
    C2 --> C2a[Output: Hello then horizontal space then World]
    C3 --> C3a[Output: C:\Users]
    C4 --> C4a[Output: He said Hi in quotes]

    D --> D1[Newline example]
    D --> D2[Tab example]
    D --> D3[Backslash example]
    D --> D4[Quotes example]
    D1 --> D1a[Console.WriteLine with \n inside string]
    D1a --> D1b[Prints Line 1 and Line 2 on separate lines]
    D2 --> D2a[Console.WriteLine with \t inside string]
    D2a --> D2b[Prints Name: then tabbed space then John]
    D3 --> D3a[Console.WriteLine with \\\\ inside string]
    D3a --> D3b[Prints C:\Program Files\App]
    D4 --> D4a[Console.WriteLine with backslash-quote inside string]
    D4a --> D4b[Prints She said Hello with surrounding quotes]

    E --> E1[Multiple escape sequences can be combined in one string]
    E1 --> E2[Example: backslash-quote, \n, and \t combined]
    E2 --> E3[Prints quoted word on first line]
    E2 --> E4[Prints indented text on second line]
```
